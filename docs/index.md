---
layout: home

hero:
  name: Ratatosk
  text: Self-hosted ngrok alternative
  tagline: Expose local servers to the internet. Open-source, fast, and yours to control.
  image:
    src: /ratatosk-logo.png
    alt: Ratatosk Logo
  actions:
    - theme: brand
      text: Get Started
      link: /guide/getting-started
    - theme: alt
      text: View on GitHub
      link: https://github.com/ragnarok22/ratatosk

features:
  - icon: "\U0001F3E0"
    title: Self-Hosted
    details: Run on your own VPS. No third-party services, no usage limits, no vendor lock-in. Your infrastructure, your rules.
  - icon: "\u26A1"
    title: Fast Multiplexing
    details: A single TCP connection handles thousands of concurrent requests using yamux multiplexing. No extra ports needed on your local router.
  - icon: "\U0001F4CA"
    title: Built-in Dashboard
    details: Monitor active tunnels, bandwidth, and real-time traffic through an embedded React dashboard. No separate install required.
---
